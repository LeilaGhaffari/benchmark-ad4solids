...
inline int SymmetricMatUnpack(const double sym[6], double full[3][3]) {
  full[0][0] = sym[0];
  full[0][1] = sym[5];
  full[0][2] = sym[4];
  full[1][0] = sym[5];
...
inline int SymmetricMatPack(const double full[3][3], double sym[6]) {
  sym[0] = full[0][0];
  sym[1] = full[1][1];
  sym[2] = full[2][2];
  sym[3] = full[1][2];
...
inline int MatInverse(const double A[3][3], double det_A, double A_inv[3][3]) {
  double B[3][3] = {
...
inline int MatMatMult(double alpha, const double A[3][3], const double B[3][3], double C[3][3]) {
  for (int j = 0; j < 3; j++) {
    for (int k = 0; k < 3; k++) {
      C[j][k] = 0;
      for (int m = 0; m < 3; m++) {
...
inline int MatComputeInverseSymmetric(const double A[3][3], const double det_A, double A_inv[6]) {
...
inline double MatDetAM1Symmetric(const double A_sym[6]) {
...
inline double MatTraceSymmetric(const double A_sym[6]) { return A_sym[0] + A_sym[1] + A_sym[2]; };


inline double MatDetAM1(const double A[3][3]) {
...
inline void DeformationGradient(double grad_u[3][3], double F[3][3]) {
  F[0][0] = grad_u[0][0] + 1.;
  F[0][1] = grad_u[0][1];
  F[0][2] = grad_u[0][2];
  F[1][0] = grad_u[1][0];
...
inline double Log1pSeries(double x) {
    double sum = 0;
    double y = x / (2. + x);
    double y2 = y*y;
    sum += y;
...
inline int LinearStrain(const double grad_u[3][3], double e_sym[6]) {
  e_sym[0] = grad_u[0][0];
  e_sym[1] = grad_u[1][1];
  e_sym[2] = grad_u[2][2];
...
inline int GreenEulerStrain(const double grad_u[3][3], double e_sym[6]) {
...
inline int GreenEulerStrain_fwd(const double grad_du[3][3], const double b[3][3], double de_sym[6]) {
  const int ind_j[6] = {0, 1, 2, 1, 0, 0}, ind_k[6] = {0, 1, 2, 2, 2, 1};
  for (int m = 0; m < 6; m++) {
    de_sym[m] = 0;
    for (int n = 0; n < 3; n++) {
...
inline double StrainEnergy(double e_sym[6], const double lambda, const double mu) {
  double e2_sym[6];
...
inline int MatMatTransposeMult(const double A[3][3], const double B[3][3], double C[3][3]) {
  for (int j = 0; j < 3; j++) {
    for (int k = 0; k < 3; k++) {
      C[j][k] = 0;
      for (int m = 0; m < 3; m++) {
...
inline void PullBack_symmetric(double Grad_u[3][3], double a_sym[6], double A_sym[6]) {
  // F = I + Grad_u
  const double F[3][3] = {
...
inline void dPullBack_symmetric(double F_inv[3][3], double dF[3][3], double a_sym[6], double da_sym[6], double dA_sym[6]) {
...
inline void PushForward_symmetric(double F[3][3], double A_sym[6], double a_sym[6]) {
...
inline void dPushForward_symmetric(const double F[3][3], double dF[3][3], double A_sym[6], double dA_sym[6], double da_sym[6]) {
  // F = I + Grad_u => dF = Grad_du
...
inline void MatTransposeMatMult(double alpha, const double A[3][3], const double B[3][3], double C[3][3]) {
  for (int j = 0; j < 3; j++) {
    for (int k = 0; k < 3; k++) {
      C[j][k] = 0;
      for (int m = 0; m < 3; m++) {
...
inline int MatMatAdd(double alpha, const double A[3][3], double beta, const double B[3][3], double C[3][3]) {
  for (int j = 0; j < 3; j++) {
    for (int k = 0; k < 3; k++) {
